pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                sh  'printenv'
                sh  'docker build -t umerakmal104/solar-system:$GIT_COMMIT .'
            }
        }
        ...
        stage('Trivy Vulnerability Scanner') {
            steps {
                sh  ''' 
                    trivy image umerakmal104/solar-system:$GIT_COMMIT \
        ...
        stage('Push Docker Image') {
            steps {
                withDockerRegistry(credentialsId: 'docker-hub-creds', url: "") {
                    sh  'docker push umerakmal104/solar-system:$GIT_COMMIT'
                }
            }
        }
